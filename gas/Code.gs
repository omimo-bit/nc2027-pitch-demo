...
function submitVisualAudit_(ss, p) {
  ...
  var snapshot = {
    ...
    engine:'ON_DEVICE_REFERENCE_ASSISTED_CV_V4'
  };
  ...
  return {ok:true, submissionId:submissionId, validationStatus:validationStatus, message:validationStatus==='VALIDATED'?'Visual audit tersimpan dan tervalidasi.':'Visual audit tersimpan dan masuk exception queue.'};
}
...
function visualAuditRowsFast_(data,limit){var ids={};data.submissions.forEach(function(s){if(s.task_id==='TASK-SOS')ids[s.submission_id]=s;});var out=[];data.versions.forEach(function(v){if(ids[v.submission_id]&&String(v.reason)==='AI_VISUAL_AUDIT'){try{var p=JSON.parse(String(v.payload_snapshot||'{}'));out.push({submissionId:v.submission_id,changedAt:v.changed_at,manual:p.manual||{},ai:p.ai||{},comparison:p.comparison||{}});}catch(ignore){}}});out.sort(function(a,b){return String(b.changedAt).localeCompare(String(a.changedAt));});return out.slice(0,limit);}
