...
function doPost(e) {
  try {
    ...
    if (!lock.tryLock(8000)) {
      return json_({ok:false, error:'BUSY', message:'Backend sedang sibuk. Coba sekali lagi.'});
    }
...
function bootstrap_(ss, p) {
  ...
  return {
    ok: true,
    user: safeUser_(user),
    stats: analyticsStats_(ss),
    trend: analyticsTrend_(ss),
    regionPerformance: regionPerformance_(ss),
    tasks: user.role_id === 'NC' ? todayTasks_(ss, user.user_id) : [],
    recentSubmissions: recentSubmissions_(ss, user, 8),
    queue: (user.role_id === 'DATA_ENTRY' || user.role_id === 'DATA_ANALYST' || user.role_id === 'PROJECT_MANAGER') ? queueRows_(ss, 20) : [],
    system: health_(ss)
  };
}
...
function recentSubmissions_(ss, user, limit) {
  ...
  return subs.slice(0, limit).map(function(s) {
    ...
    return {
      ...
      task:s.task_id === 'TASK-STOCK' ? 'Stock Taking' : (s.task_id === 'TASK-OFFTAKE' ? 'Offtake' : s.task_id),
